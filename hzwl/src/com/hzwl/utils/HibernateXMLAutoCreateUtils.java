...
public class HibernateXMLAutoCreateUtils
{

	/**
	 * 
	 * @function 根据POJO类生成简单的相应*.hbm.xml配置文件，复杂功能请在此基础上手动配置。<br/>
	 ...
	 *            数据库中的相应表名
	 */
	public static void createHibernatePOJOXML(Class<?> cls,
			String identification, String tableName)
	{

		Document rootDocument = DocumentHelper.createDocument();
		rootDocument.addDocType("hibernate-mapping",
		...
		for (Field field : declaredFields)
		{
			...
			if (field.getName().equals(identification))
			{
				...
				Element ElementClassIDColumn = ElementClassID
						.addElement("column");
				ElementClassIDColumn.addAttribute("name", identification);
				Element ElementClassIDGenerator = ElementClassID
		...
		{
			System.out
					.println("HibernateXMLAutoCreateUtils:XML生成错误,看到这个绝壁是因为长得丑");
			e.printStackTrace();
		}
	...
	/**
	 * 
	 * @function 使用此方法配置文件中的字符集必须是GBK,否则会出现解码问题。<br/>
	 *           此方法可以在生成配置文件的同时关联主配置文件映射<br/>
	 *           根据POJO类生成简单的相应*.hbm.xml配置文件，复杂功能请在此基础上手动配置。<br/>
	 *           (Tip:请在完成POJO后使用junit或者main方法预先执行此方法以生成配置文件)
	 * @time 2016-1-13 下午03:48:20
	 * @param cls
	 *            POJO的类对象
	 ...
	 *            数据库中的相应表名
	 */
	public static void createHibernatePOJOMapping(Class<?> cls,
			String identification, String tableName)
	{
		...
		{
			System.out
					.println("[createHibernatePOJOMapping]说了主配置文件要用GBK编码格式，孩子就是不听话");
			System.out
					.println("[createHibernatePOJOMapping]删除主配置文件然后再添加后修改配置文件编码集为GBK");
			e2.printStackTrace();
		}

		List<?> selectNodes = doc
				.selectNodes("/hibernate-configuration/session-factory/mapping/@resource");
		boolean flag = false;
	 ...
	 * @param Many_POJO
	 *            N-1关系N端的类对象
	 * @param Many_POJOIdentification
	 *            N-1关系N端对象对应表中主键所对应的属性名
	 * @param Many_POJO_tableName
	 *            N-1关系N端对象对应的表名
	 * @param One_POJO
	 *            N-1关系1端的类对象
	 * @param One_POJOIdentification
	 *            N-1关系1端对象对应表中主键所对应的属性名
	 * @param One_POJO_tableName
	 *            N-1关系1端对象对应的表名
	 */
	public static void createHibernatePOJOMappingManyToOne(Class<?> Many_POJO,
			String Many_POJOIdentification, String Many_POJO_tableName,
			Class<?> One_POJO, String One_POJOIdentification,
			String One_POJO_tableName)
	{

		Document rootDocumentPOJO1 = DocumentHelper.createDocument();
		rootDocumentPOJO1.addDocType("hibernate-mapping",
				"-//Hibernate/Hibernate Mapping DTD 3.0//EN",
				"http://hibernate.sourceforge.net/hibernate-mapping-3.0.dtd");
		// 加入根
		Element rootElement1 = rootDocumentPOJO1
				.addElement("hibernate-mapping");
		// 在根中加第一个子节点
		...
		for (Field field : declaredFields1)
		{
			...
			if (field.getName().equals(Many_POJOIdentification))
			{
				...
				Element ElementClassIDColumn = ElementClassID1
						.addElement("column");
				ElementClassIDColumn.addAttribute("name",
						Many_POJOIdentification);
				Element ElementClassIDGenerator = ElementClassID1
						.addElement("generator");
				ElementClassIDGenerator.addAttribute("class", "native");
			} else if (field.getType().getSimpleName()
					.equals(One_POJO.getSimpleName()))
			{
		...
		rootDocumentPOJO2.addDocType("hibernate-mapping",
				"-//Hibernate/Hibernate Mapping DTD 3.0//EN",
				"http://hibernate.sourceforge.net/hibernate-mapping-3.0.dtd");
		// 加入根
		Element rootElement2 = rootDocumentPOJO2
				.addElement("hibernate-mapping");
		// 在根中加第一个子节点
		...
		for (Field field : declaredFields2)
		{
			...
			if (field.getName().equals(One_POJOIdentification))
			{
				...
				Element ElementClassIDColumn = ElementClassID2
						.addElement("column");
				ElementClassIDColumn.addAttribute("name",
						One_POJOIdentification);
				Element ElementClassIDGenerator = ElementClassID2
						.addElement("generator");
				ElementClassIDGenerator.addAttribute("class", "native");
		...
		Writer writer1 = null;
		Writer writer2 = null;
		String pathWay1 = "src/" + Many_POJO.getName().replace('.', '/')
				+ ".hbm.xml";
		String pathWay2 = "src/" + One_POJO.getName().replace('.', '/')
				+ ".hbm.xml";
		try
		{
		...
		if (!flagPOJO1)
		{
			...
			Element mappingElementForAdd = sessionFactoryNode
					.addElement("mapping");
			mappingElementForAdd
					.addAttribute("resource", pathWay1.substring(4));
		...
		if (!flagPOJO2)
		{
			...
			Element mappingElementForAdd = sessionFactoryNode
					.addElement("mapping");
			mappingElementForAdd
					.addAttribute("resource", pathWay2.substring(4));
	 ...
	 *            关系表的表名
	 */
	private static Document createHibernatePOJOMappingManyToManyComponent(
			...
			String MappingTableName)
	{
		...
		rootDocumentPOJO.addDocType("hibernate-mapping",
				"-//Hibernate/Hibernate Mapping DTD 3.0//EN",
				"http://hibernate.sourceforge.net/hibernate-mapping-3.0.dtd");
		// 加入根
	 ...
	 *           此方法可以在生成配置文件的同时关联主配置文件映射<br/>
	 *           根据POJO类生成简单的相应*.hbm.xml配置文件，复杂功能请在此基础上手动配置。<br/>
	 *           (Tip:请在完成POJO后使用junit或者main方法预先执行此方法以生成配置文件) <br/>
	 *           使用此方法可以根据指定的POJO和表名来创建多对多关系映射文件。<br/>
	 *           TIP：使用此方法生成的映射文件默认lazy='false' <br/>
	 *           如果需要重复生成的话,请删除自己手动添加的属性类型后再生成
	 * @time 2016-1-13 下午03:48:20
	 * @param ManyPOJO1
	 *            多对多关联的第一个类
	 * @param identification1
	 ...
	 * 
	 */
	public static void createHibernatePOJOMappingManyToMany(Class<?> ManyPOJO1,
			String identification1, String tableName1, Class<?> ManyPOJO2,
			String identification2, String tableName2, String MappingTableName)
	{

		Document rootDocumentPOJO1 = createHibernatePOJOMappingManyToManyComponent(
				ManyPOJO1, identification1, tableName1, ManyPOJO2,
				identification2, tableName2, MappingTableName);
		Document rootDocumentPOJO2 = createHibernatePOJOMappingManyToManyComponent(
				ManyPOJO2, identification2, tableName2, ManyPOJO1,
				identification1, tableName1, MappingTableName);

		// 写xml

		Writer writer1 = null;
		Writer writer2 = null;
		String pathWay1 = "src/" + ManyPOJO1.getName().replace('.', '/')
				+ ".hbm.xml";
		String pathWay2 = "src/" + ManyPOJO2.getName().replace('.', '/')
				+ ".hbm.xml";
		try
		{
			writer1 = new FileWriter(pathWay1);
