...
<script type="text/javascript">
	...
	function doEdit(){
		...
		if (rows.length == 1) {
			...
			for (var i = 0; i < rows.length; i++) {
				...
				/* $("input[name=staff.id]").val(rows[i].Staff.name); */
				$("#editDecidedzoneForm").form("load", rows[i]);
			} 
	...
	function doDelete(){
		...
		if (rows.length == 0) {
			...
			var ids = array.join(",");
			//发送请求，传递ids参数
			window.location.href = '${pageContext.request.contextPath}/decide_deleteDecidedzone.action?ids='
					+ ids;
		}
	}
	...
	function doAssociations(){
		...
		if(rows.length == 1){
			...
			var url1 = "${pageContext.request.contextPath}/decide_findNoAssociationCustomers.action";
			...
			var url2 = "${pageContext.request.contextPath}/decide_findAssociationCustomers.action";
	...
	$(function(){
		...
		$('#grid').datagrid( {
			...
			pagination : true,
			toolbar : toolbar,
			url : "${pageContext.request.contextPath}/decide_pageQuery.action",
			idField : 'id',
			columns : columns,
	...
	function doDblClickRow(rowIndex, rowData){
		...
		$('#association_subarea').datagrid( {
			...
			striped : true,
			queryParams: {
				decidedzoneId: rowData.id
			},
			url : "${pageContext.request.contextPath}/subarea_assoctionDecidedzone.action",
			columns : [ [{
				field : 'id',
				title : '分拣编号',
				width : 120,
		...
		$('#association_customer').datagrid( {
			...
			url : "${pageContext.request.contextPath}/decide_findAssociationCustomers.action",
			columns : [[{
				field : 'id',
				title : '客户编号',
				width : 120,
...
<body class="easyui-layout" style="visibility:hidden;">
	...
	<div class="easyui-window" title="定区添加修改" id="addDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="addDecidedzoneForm" action="${pageContext.request.contextPath }/decide_addDecidedzone.action"
					method="post">
						...
						<td>
							<input class="easyui-combobox" name="staff.id"  
    							data-options="valueField:'id',textField:'name',
    								url:'${pageContext.request.contextPath }/staff_listStaff.action'" />  
						</td>
					</tr>
					<tr height="300">
						...
						<td>
							<table id="subareaGrid"  class="easyui-datagrid"
								 border="false" style="width:300px;height:300px"
								  data-options="url:'${pageContext.request.contextPath }/subarea_listSubarea.action',
	...
	<div class="easyui-window" title="定区添加修改" id="editDecidedzoneWindow" collapsible="false" minimizable="false" maximizable="false" style="top:20px;left:200px">
		...
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="editDecidedzoneForm" action="${pageContext.request.contextPath }/decide_update.action"
					method="post">
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
						<td colspan="2">定区信息</td>
	...
	<div class="easyui-window" title="关联客户窗口" modal="true" id="customerWindow" collapsible="false" closed="true" minimizable="false" maximizable="false" style="top:20px;left:200px;width: 400px;height: 300px;">
		<div style="overflow:auto;padding:5px;" border="false">
			<form id="customerForm" action="${pageContext.request.contextPath }/decide_assignCustomersToDecidedZone.action" method="post">
				<table class="table-edit" width="80%" align="center">
					<tr class="title">
